...
kuka_sphere_object_shape_detection::kuka_sphere_object_shape_detection()
{
    ...
    nnh.param("workspace_limits", workspace_limits,
              std::string("0.1 0.5 -0.35 0.35 0.9 1.6"));  // xmin xmax ymin ymax zmin zmax
    parseWorkspaceLimits(workspace_limits);

    nnh.param("shape_detection_workspace_limits", shape_detection_workspace_limits,
              std::string("0.2 0.4 -0.25 0.25 0.95 1.45"));  // xmin xmax ymin ymax zmin zmax
...
void kuka_sphere_object_shape_detection::checkWorkspace(const tf::Vector3 spherePosition)
{
    ...
    {
        insideShapeDetection = false;
        ROS_ERROR("y < ymin: modechange true");
        leftMode = false;
        modeChange = true;
        firstItCMRM = true;
    ...
    {
        insideShapeDetection = false;
        ROS_ERROR("y > ymax: modechange true");
        leftMode = true;
        modeChange = true;
...
geometry_msgs::Vector3Stamped kuka_sphere_object_shape_detection::TranslationRuleB(tf::Vector3 forceVec)
{
    ...
    if (std::abs(forceVec.getY()) > std::abs(forceVec.getZ()) || std::abs(forceVec.getZ()) > std::abs(forceVec.getY()))
    {
        ...
        if (std::abs(forceVec.getY()) > std::abs(forceVec.getZ()))
        {
            topForce = forceVec.getY();
            weakerForce = forceVec.getZ();
        }
        else
        {
            topForce = forceVec.getZ();
            weakerForce = forceVec.getY();
        }
        if (topForce < 0)
        {
            topForce *= -1;
    ...
    if (leftMode)  // to the left
    {
        if (forceVec.getY() > 0 && forceVec.getZ() < 0)  // ->v
        {
            forceVec[1] = (topForce - FORCE_THRESHOLD) * 0.002;
            forceVec[2] = (topForce - FORCE_THRESHOLD) * relativeWeakForce * 0.002 * -1;
        }
        else if (forceVec.getY() > 0 && forceVec.getZ() > 0)  // ->^
        {
            forceVec[1] = (topForce - FORCE_THRESHOLD) * relativeWeakForce * 0.002 * -1;
...
void kuka_sphere_object_shape_detection::changeModeResetMotion()
{
    ...
    {
        OneTimeBool = true;
        spherePositionTransformGoal.getOrigin().setX(spherePositionTransformGoal.getOrigin().getX() + 0.01);
    ...
    if (leftMode)
    {
        if (OneTimeBool)
        {
            ...
            spherePositionTransformGoal.getOrigin().setY(spherePositionTransformGoal.getOrigin().getY() - 0.02);
            OneTimeBool = false;
        }
    }
    else if (!leftMode)
...
void kuka_sphere_object_shape_detection::searchObjectGoal()
{
    ...
    if (!currentXInitialized)
    {
        currentX = spherePositionTransform.getOrigin().getX();
        currentXInitialized = true;
    }
        ...
        -FORCE_THRESHOLD > externalTCPWrenchVec[1] || -FORCE_THRESHOLD > externalTCPWrenchVec[2])
    {
        ...
        PointOnSphereVec[0] = 0;                      // VecSpherePoint.getX();//SpherePoint.getX();
        PointOnSphereVec[1] = VecSpherePoint.getY();  // SpherePoint.getY();
        PointOnSphereVec[2] = VecSpherePoint.getZ();  // SpherePoint.getZ();
        ...
        whatevB.vector.x = currentX;
        tf::Vector3 whatevBOrigin;
        whatevBOrigin[0] = 0;
        whatevBOrigin[1] = whatevB.vector.y;
    ...
    else
    {
        ...
        if (leftMode)
        {
            spherePositionTransform.getOrigin().setY(spherePositionTransform.getOrigin().getY() - 0.001);
        }
        else if (!leftMode)
        {
            spherePositionTransform.getOrigin().setY(spherePositionTransform.getOrigin().getY() + 0.001);
        }
        spherePositionTransform.getOrigin().setX(currentX);
...
void kuka_sphere_object_shape_detection::publishStartGoal()
{
    ...
    // -1.9058869195474655, -0.27576307740655404 (wsg angles: -0.02885, 0.02885 -> open?)
    double startAngles[] = { 1.72, 1.075, 1.15, 1.45, 0.244, -1.906, -0.276 };
