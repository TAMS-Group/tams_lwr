...
#define FRI_MSR_DATA_SIZE                                                                                              \
    (FRI_HEADER_SIZE + FRI_KRL_DATA_SIZE + FRI_INTF_STATE_SIZE + FRI_ROBOT_STATE_SIZE + FRI_ROBOT_DATA_SIZE)
...
#define FRI_CHECK_SIZES_OK                                                                                             \
    ((sizeof(tFriHeader) == FRI_HEADER_SIZE) && (sizeof(tFriKrlData) == FRI_KRL_DATA_SIZE) &&                          \
     (sizeof(tFriIntfStatistics) == FRI_INTF_STATISTICS_SIZE) && (sizeof(tFriIntfState) == FRI_INTF_STATE_SIZE) &&     \
     (sizeof(tFriRobotState) == FRI_ROBOT_STATE_SIZE) && (sizeof(tFriRobotData) == FRI_ROBOT_DATA_SIZE) &&             \
     (sizeof(tFriRobotCommand) == FRI_ROBOT_COMMAND_SIZE) && (sizeof(tFriMsrData) == FRI_MSR_DATA_SIZE) &&             \
...
#define FRI_CHECK_BYTE_ORDER_OK                                                                                        \
    ((_friTestByteOrder.a == 0x40490FDB) && (_friTestByteOrder.b[0] == 0xDB) && (_friTestByteOrder.b[1] == 0x0F) &&    \
     (_friTestByteOrder.b[2] == 0x49) && (_friTestByteOrder.b[3] == 0x40) && (_friTestByteOrder.c > 3.141592) &&       \
