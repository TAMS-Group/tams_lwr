...
//! <li>Creating a communication thread that is dedicated for all UDP communication procedures to exchange data between
//! the KRC unit and the remote host. <li>Creating a timer thread that periodically send signal through a condition
//! variable <li>As soon as the communication thread cyclically receives data telegrams from the KRC unit: <ul>
//! <li>Providing robot status information (e.g., temperature, arm power state, errors, warnings, etc.) <li>Providing
//! robot control data (e.g., measured joint position vector, measured joint torque vector, etc.) <li>Providing shared
//! KRL variables <li>Providing communication status and quality data (e.g., jitter, latency, etc.; measured by the KRC
//! host) <li>Setting robot control data (e.g., commanded joint positions, commanded joint torques, etc.) <li>Setting
//! shared KRL variables <li>Performing the start-up procedure of the robot <li>Performing the shutdown procedure of the
//! robot <li>Enabling low-level real-time data logging methods <li>Providing two different timer signals (KRC timer and
//! local timer) <li>A real-time capable method for console output
//! </ul>
//! </ul>
//! Users may use this class as the interface to the KRC unit if the complete functionality is required for the desired
//! application. In order to provide a simple and easy-to-start-with interface, this class is used by (but not derived
//! to) the classes\n <ul> <li>LWRJointPositionController for the joint position controller,
//! <li>LWRCartImpedanceController for the Cartesian impedance controller, <li>LWRJointImpedanceController for the joint
//! impedance controller, and <li>LWRJointTorqueController for the joint torque controller,\n
//! </ul>
...
public:
    ...
    //! <li> the initialization file specified by \c InitFileName does not contain all required parameters (cf. \ref
    //! sec_InitFile), or <li> one of the threads FastResearchInterface::KRCCommunicationThreadMain() or
    //! FastResearchInterface::TimerThreadMain() cannot be created successfully
    //!	</ul>
    ...
    //! </ol>\n
    //! The complete start-up procedure must not take longer than \c TimeOutValueInSeconds seconds. If completed
    //! successfully, the robot arm is ready for operation. This method should be used pairwise with the method
    ...
    //! <li> whether the KRC node is configured correctly in particular w.r.t. the network settings (cf. \ref
    //! sec_Introduction).
    //! </ul>
    //! <li> \c EALREADY if the KRC unit is already in command mode, that is, the KRL function <tt>friStart()</tt> was
    //! already called by a previous call of FastResearchInterface::StartRobot(). If the robot is not ready for operation
    //! (i.e., the result of FastResearchInterface::IsMachineOK() is \c false), call FastResearchInterface::StopRobot()
    //! first. It might be required to check whether the KRC unit is running correctly, and whether the KRL program KRL
    //! program \c FRIControl (cf.\ref sec_KRLFile1) is being executed. <li> \c ETIME if the start-up procedure could not
    //! be completed within the specified time interval of \c TimeOutValueInSeconds <li> \c EOK if no error occurred.
    //! </ul>
    //!
    ...
    //! <li> \c ENOTCONN if no connection between the remote host and the KRC unit could be established. Please check
    //! <ul>
    //! <li> whether the KRC unit is turned on and the Ethernet cable (e.g., a crossed cable) is plugged in on both
    //! nodes, <li> whether the KRL program \c FRIControl (cf.\ref sec_KRLFile1) has been started, or <li> whether the
    //! KRC node is configured correctly in particular w.r.t. the network settings (cf. \ref sec_Introduction).
    //! </ul>
    ...
    //! <li><b>After calling the KRL function <tt>friStop()</tt>, the robot arm power may be still turned on. In the
    //! cases of <ul> <li>FastResearchInterface::CART_IMPEDANCE_CONTROL,
    //! <li>FastResearchInterface::JOINT_IMPEDANCE_CONTROL, or <li>FastResearchInterface::JOINT_TORQUE_CONTROL,
    //! </ul>
    ...
    //!
    //! \brief
    //! Copies one single floating point value into the data telegram to be send to the KRC unit and read by the KRL
    //! program from the <tt>\$FRI_FRM_REAL[<b>Index</b>]</tt>
    //!
    //! \param Index
    ...
    //!
    //! \param Value
    //! The value to be copied into the data telegram and to read by the KRL program as
    //! <tt>\$FRI_FRM_REAL[<b>Index</b>]</tt>.
    //!
    //! \sa tFriCmdData
    //! \sa FastResearchInterface::SetKRLFloatValues()
    //! \sa FastResearchInterface::GetKRLFloatValue()
...
protected:
    ...
    //!
    //! \brief
    //! Condition variable used by the KRC communication thread FastResearchInterface::KRCCommunicationThreadMain() for
    //! broadcasting after the reception of a message from the KRC unit
    //  ----------------------------------------------------------
