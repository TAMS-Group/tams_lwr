...
public:
    ...
    /*TODO if it work adapt subscribers!!
      FRI_->SetCommandedJointStiffness(commandedStiffness_);
      FRI_->SetCommandedJointDamping(commandedDamping_);
      FRI_->SetCommandedJointTorques(commandedTorquesInNm_);
    */
    ...
    void executeCartImpedanceTrajectory()
    {
        if (goal_active_)
        {
            ...
            for (unsigned int i = 0; i < g->trajectory.points.size(); i++)
            {
                ...
                {
                    request.fk_link_names.push_back(names[j]);
                    request.robot_state.joint_state.name.push_back(names[j]);
                ...
                ROS_INFO("position_x %f", response.pose_stamped[0].pose.position.x);
                ROS_INFO("position_y %f", response.pose_stamped[0].pose.position.y);
                ROS_INFO("position_z %f", response.pose_stamped[0].pose.position.z);
                ROS_INFO("orientation_x %f", response.pose_stamped[0].pose.orientation.x);
                ROS_INFO("orientation_y %f", response.pose_stamped[0].pose.orientation.y);
                ROS_INFO("orientation_z %f", response.pose_stamped[0].pose.orientation.z);
                ROS_INFO("orientation_w %f", response.pose_stamped[0].pose.orientation.w);
    ...
    void setJointDamping(const std_msgs::Float32MultiArray::ConstPtr& msg)
    {
        ...
        {
            float commandedDamping[LBR_MNJ];
            for (int i = 0; i < LBR_MNJ; i++)
            {
    ...
    void setJointStiffness(const std_msgs::Float32MultiArray::ConstPtr& msg)
    {
        if (FRIMode_ != FastResearchInterface::JOINT_POSITION_CONTROL)
        {
            float commandedStiffness[LBR_MNJ];
            for (int i = 0; i < LBR_MNJ; i++)
            {
    ...
    void setJointTorques(const std_msgs::Float32MultiArray::ConstPtr& msg)
    {
        if (FRIMode_ != FastResearchInterface::JOINT_POSITION_CONTROL)
        {
            float commandedTorques[LBR_MNJ];
            for (int i = 0; i < LBR_MNJ; i++)
            {
                commandedTorques[i] = msg->data[i];
            }
            FRI_->SetCommandedJointTorques(commandedTorques);
        }
    }
...
// rostopic pub /lwr_setFRIMode std_msgs/UInt32 "data: 10"
