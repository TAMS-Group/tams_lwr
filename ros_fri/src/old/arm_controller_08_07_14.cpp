...
public:
    ...
    void stopLwr(Goal g)
    {
        ...
        for (unsigned int j = 0; j < LBR_MNJ; j++)
        {
            IP_->CurrentPosition->VecData[j] = (double)DEG(measuredJointPositions_[j]);
        ...
        for (unsigned int j = 0; j < 1; j++)
        {
            ...
            for (unsigned int j = 0; j < LBR_MNJ; j++)
            {
                commandedJointPositions_[j] = RAD((double)(OP_->NewPosition->VecData[j]));
            }
            FRI_->SetCommandedJointPositions(commandedJointPositions_);
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        switch (lwr_action_server.getCurrentControlScheme())
        {
            ...
            default:
                ROS_INFO("lwr_action_server ERROR: Not supported CurrentControlScheme! Select JOINT_POSITION_CONTROL, "
